...
class HomeController extends Controller
{
  public function getDefaultInformation(){
    ...
    return view('vista1', ['users'=>$users]);
  }
  public function goLoginByAjax(Request $request){
    $input = $request->all();
    $login = new Login;
    $login->user_name = $input['user'];
    $login->user_password = $input['password'];
    $login->save();

    //----------------------------------------
    $user = User::where('user_login', $input['user'])->first();
    if (empty($user)) {
      return response()->json(['success'=>'Coincidencia no encontrada']);
    }else{
      if ($input['password'] == $user->user_password) {
        session(['id' => $user->id]);
